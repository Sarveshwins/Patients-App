...
const HaveSection = ({
    ...
    if (['19', '20'].includes(item.key)) {
      return (
        <TouchableHighlight
          ...
          onPress={() => {
            handleTabClickHave('Have', item), ref2.current.open();
          }}>
  ...
  const [filteredData, setFilteredData] = useState(AllergiesList);
  ...
  const data = defaultAnswersHave[selectedItemHave?.key];
  console.log('key', defaultAnswersHave[selectedItemHave?.key], data);
  const poweredBy = () => {
    return (
      <View
        style={{
          ...
          <FlatList
            ...
            renderItem={({item, index}) => (
              <TouchableHighlight
                ...
                onPress={() => {
                  setAnswerHave(item);
                  setSelectedDefaultAnswerHave(item);
                }}>
                <View
                  style={{
                    flexDirection: 'row',
                  ...
                  {selectedDefaultAnswerHave === item && (
                    <Text
                      style={{
                        color: '#000000',
                        fontSize: 18,
